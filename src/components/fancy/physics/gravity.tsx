...
export const MatterBody = ({
  ...
  return (
    <div
      ref={elementRef}
      className={`absolute w-max${isDraggable ? " pointer-events-none" : ""}${className ? ` ${className}` : ""}`}
    >
      {children}
    </div>
  )
  ...
  (
    ...
    return (
      <GravityContext.Provider value={{ registerElement, unregisterElement }}>
        <div
          ref={canvas}
          className={`stack-gravity absolute inset-0 h-full w-full${className ? ` ${className}` : ""}`}
          {...props}
        >
          {children}
        </div>
